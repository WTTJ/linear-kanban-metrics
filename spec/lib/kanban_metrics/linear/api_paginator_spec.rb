  ...
  describe 'private methods' do
    describe '#normalize_page_info' do
      ...
      context 'with complete page info' do
        # Setup
        let(:page_info) { { 'hasNextPage' => true, 'endCursor' => 'test-cursor' } }

        it 'normalizes complete page info correctly' do
          # Execute
          result = normalize_page_info
      ...
      context 'with missing hasNextPage' do
        # Setup
        let(:page_info) { { 'endCursor' => 'test-cursor' } }

        it 'defaults hasNextPage to false' do
          # Execute
          result = normalize_page_info

          # Verify
          aggregate_failures 'missing hasNextPage handling' do
            expect(result[:has_next_page]).to be false
            expect(result[:end_cursor]).to eq('test-cursor')
          end
        end
      end

      context 'with empty page info' do
        # Setup
        let(:page_info) { {} }

        it 'handles empty page info gracefully' do
          # Execute
          result = normalize_page_info
    ...
    describe '#log_page_fetch' do
      subject(:log_page_fetch) { paginator.send(:log_page_fetch, page_number, total_results) }
      ...
      context 'with debug mode enabled' do
        ...
        it 'logs page fetch in debug mode' do
          # Execute & Verify
          expect { log_page_fetch }.to output(/Fetching page 2/).to_stdout
        end
      end

      context 'with large dataset (no debug)' do
        # Setup
        let(:page_number) { 2 }
        ...
        it 'logs page fetch for large datasets' do
          # Execute & Verify
          expect { log_page_fetch }.to output(/Fetching page 2/).to_stdout
        end
      end

      context 'with small dataset (no debug)' do
        # Setup
        let(:page_number) { 2 }
        let(:total_results) { 100 }
        ...
        it 'does not log for small datasets without debug' do
          # Execute & Verify
          expect { log_page_fetch }.not_to output.to_stdout
        end
      end
    end
  end
